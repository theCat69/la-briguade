...
Seven categories the agent must scan for in every changed file:
...
7. **Weak test coverage** — behavior not locked by any test, assertions that only check "no error thrown" without verifying the actual result, missing edge cases on code paths touched in cleanup passes.
...
**Pass 2 — Duplication + needless abstraction**
Extract repeated logic into a single authoritative location. Remove copy-paste branches and pass-through wrappers. Consolidate redundant helpers. Only extract when duplication is exact and the knowledge is the same — do not merge code that merely looks structurally similar but serves different concerns.

**Pass 3 — Local clarity + boundaries**
Rename generic identifiers (`data`, `value`, `temp`, `result`, `obj`, `info`) to intention-revealing names. Ensure errors are explicit and typed — no silent swallowing, no mixed return/error values. Remove noise comments (inline "what" comments, any commented-out code that survived Pass 1). Fix only local, in-scope boundary violations; flag architectural or cross-scope boundary fixes rather than expanding the change.
...
- **Lock behavior with tests BEFORE deleting anything that has side effects.** Write the test first, then delete. If test writing is not enabled, flag the deletion and do not make it.
- **Validate the changed behavior.** Run the project-relevant checks when available. If validation cannot run, report the exact missing prerequisite and remaining risk.
