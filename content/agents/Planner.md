...
1. Load skill `project-coding`. Reference its project-specific conventions (TypeScript patterns, naming conventions, commit format) when evaluating feature proposals for fit with the existing codebase.
2. Load skill `project-code-examples`. Reference it to point to existing code patterns when clarifying implementation expectations.
3. Load skill `cache-ctrl-caller`. Use it to check cache state before calling local-context-gatherer or external-context-gatherer.
